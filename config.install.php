...
//~ 	$bdd->query("CREATE TABLE IF NOT EXISTS `".$site_bdd_name."`.".$Table." (`id` INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `user` VARCHAR(50) NOT NULL, UNIQUE(`user`), `password` VARCHAR(255) NOT NULL, `name` VARCHAR(50) NOT NULL, `mail` VARCHAR(255)) ENGINE = MYISAM;");
	...
	echo "Creating table: '$Table'...<br>";
	$bdd->query("CREATE TABLE IF NOT EXISTS `".$site_bdd_name."`.".$Table." (`id` INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `status` INT(11), `date` DATETIME, `timeout` INT(11) NOT NULL DEFAULT '60', `suite` VARCHAR(30), `theme` VARCHAR(30), `anonymous` BOOLEAN NOT NULL DEFAULT FALSE) ENGINE = MYISAM;");
